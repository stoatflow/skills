...
| **`TopologyConfig` / `processor.wrapper.class`** (KSC-77) | `new StreamsBuilder(new TopologyConfig(config))`; `processor.wrapper.class` (KIP-1112) | `TopologyConfig` → `io.stoatflow.core.config.TopologyConfig` (projects build-time defaults only; not a Kafka `AbstractConfig`) | Usually no change for the ctor. A set **`processor.wrapper.class` throws** `TopologyValidationException` at config ingestion (no processor-wrapper SPI — deliberate divergence): remove it, or apply the wrapper logic in your processors |
